...
function App() {
  const getDefaultAcademicInfo = () => {
    ...
    let ay, sem;
    if (month >= 8) { ay = year - 1911; sem = 1; }
    else if (month === 1) { ay = year - 1911 - 1; sem = 1; }
    else { ay = year - 1911 - 1; sem = 2; }
    return { ay, sem };
  };
  ...
  const saveClassConfig = async (updates) => {
    if (!user?.uid || !currentClass) return;
    const metaRef = doc(db, `users/${user.uid}/metadata`, `${viewPrefix}_${currentClass}`);
  ...
  const handleScoreChange = async (sid, hid, val) => {
    ...
    const scoreRef = doc(db, `users/${user.uid}/scores`, `${viewPrefix}_${currentClass}`);
    await setDoc(scoreRef, { [sid]: { ...(scores[sid] || {}), [hid]: fv } }, { merge: true });
  };
  ...
  const handleExcelImport = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      for (const name of wb.SheetNames) {
        ...
        await setDoc(doc(db, `users/${user.uid}/metadata`, `${importPrefix}_${name}`), { 
          students, 
          examHeaders: DEFAULT_HEADERS,
          weights: DEFAULT_WEIGHTS,
          examVisibility: DEFAULT_VISIBILITY,
          pickBestCount: 3,
          isPaddingZero: false
        }, { merge: true });
      ...
      customSidebar={
        <div className="space-y-4 font-sans pb-10">
          ...
          <div className="grid grid-cols-2 gap-2 font-black">
            <button onClick={() => { 
              if (!currentClass) return alert("請選班級"); 
